 ...
 */
hxtp::HxtpCapabilityResult handle_set_pin(
    ...
    void* user_ctx)
{
    ...
    hxtp::HxtpCapabilityResult result;
    ...
    if (!has_pin || !has_value) {
        ...
        result.error_code = static_cast<int16_t>(hxtp::HxtpError::INVALID_PARAMS);
 ...
 */
hxtp::HxtpCapabilityResult handle_get_status(
    const char* params_json,
    uint32_t params_len,
    void* user_ctx)
{
...
void on_error(hxtp::HxtpError err, const char* msg, void* ctx) {
    (void)ctx;
...
void setup() {
    ...
    /* ── Initialize ──────────────────────────────────── */
    hxtp::HxtpError err = client->begin();
    if (err != hxtp::HxtpError::OK) {
...
void loop() {
    ...
    if (client->isConnected()) {
        ...
        if (now - last_telemetry_ms >= TELEMETRY_INTERVAL_MS) {
            ...
            if (len > 0) {
                hxtp::HxtpError err = client->publishTelemetry(json, static_cast<uint32_t>(len));
                if (err == hxtp::HxtpError::OK) {
